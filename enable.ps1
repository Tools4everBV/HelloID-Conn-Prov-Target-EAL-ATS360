#################################################
# HelloID-Conn-Prov-Target-EAL-ATS360-Enable
# PowerShell V2
#################################################
...
try {
    ...
    switch ($action) {
        'EnableAccount' {
            $body = @{
                ...
                isBlocked      = $false
                validityPeriod = @{
                    start      = (Get-Date).ToString('yyyy-MM-ddTHH:mm:ss.fffffffK')
                    expiration = (Get-Date -Year 2049 -Month 12 -Day 31).ToString('yyyy-MM-ddTHH:mm:ss.fffffffK')
                }
            }
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Enabling EAL-ATS360 account with accountReference: [$($actionContext.References.Account)]"
                $null = Invoke-RestMethod @splatEnableUserParams
            } else {
                Write-Information "[DryRun] Enable EAL-ATS360 account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditLogMessage = "Could not enable EAL-ATS360 account. Error: $($errorObj.FriendlyMessage)"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    } else {
        $auditLogMessage = "Could not enable EAL-ATS360 account. Error: $($_.Exception.Message)"
