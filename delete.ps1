##################################################
# HelloID-Conn-Prov-Target-EAL-ATS360-Delete
# PowerShell V2
##################################################
...
try {
    ...
    switch ($action) {
        'DeleteAccount' {
            $splatDeleteUserParams = @{
                Uri    = "$($actionContext.Configuration.BaseUrl)/v1.1/BadgeHolder($($actionContext.References.Account))"
                Method = 'Delete'
            }

            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Deleting EAL-ATS360 account with accountReference: [$($actionContext.References.Account)]"
                $null = Invoke-RestMethod @splatDeleteUserParams
            } else {
                Write-Information "[DryRun] Delete EAL-ATS360 account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditLogMessage = "Could not delete EAL-ATS360 account. Error: $($errorObj.FriendlyMessage). Action initiated by: [$($actionContext.Origin)]"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    } else {
        $auditLogMessage = "Could not delete EAL-ATS360 account. Error: $($_.Exception.Message). Action initiated by: [$($actionContext.Origin)]"
